...
\noindent{\it Deception prediction}  
...
\noindent{\it Sincerity predicition}
...
\noindent{\it Sincerity prediction}
...
In the stacked generalized framework a deception prediction SVM model is first trained from the original acoustic features on the training set (this model is same as baseline deception predictio model).
Then, applytig this model on the development set, we obtain the distances from SVM decision hyperplanes as soft deception scores. 
We then train a final SVM model that combines these soft deception scores and the sincerity scores obtained on the development partition of the DSD dataset to provide the final label. 
...
The second layer SVM model is trained on the development partition of DSD dataset to prevent overfitting to the training set. 
...
An first layer SVR model predicts the sincerity scores based on the original features and is trained on the training partition of the SSC dataset (this model is same as the baseline sincerity prediction model). 
A second layer SVR model then combines the first layer predictions with the deception scores (distance from SVM hyperplanes) to provide the final sincerity scores.  
The second layer SVR model is trained on the development set to prevent overfitting to the training set. 
In the next section, we combined the appending datapoints and appending sincerity/deception outcomes as features. 

\subsection{KT: appending datapoints + using sincerity/deception predictions as features}
In this section, we combine the previous two KT approaches of appending datapoints and sincerity/deception predictions as features.
We describe the approaches for deception and sincerity prediction below.
\\

\noindent{\it Deception prediction} This experiment is similar to the deception prediction in the last section involving the incorporation of sincerity/deception scores as features during prediction.
The only difference is in the first stage of stacked generalization where we append the sincerity dataset instead to the training partition of the DSD dataset to train a deception prediction SVM model, SVM$_\text{D1}$. 
SVM could then be trained using either RANSAC or label transformation.
The second stage SVM is then trained using the hyperplane distances from the first  
...
\section{Results}
Table~\ref{} and ~\ref{} show the UARs for the various deception and sincerity prediction results. 
Apart from presenting the result for each approach (baseline, RANSAC, label transformation and appending sincerity/deception scores as features), we also perform an experiment where the training approach is selected based on the perf  
...
\begin{tabular}{l|l|l}
            & Deception (UAR) & Sincerity ($\rho$) \\ \hline
Baseline & 69.64 & 46.51 \\ \hline
\multicolumn{3}{c} {KT: adding datapoints from other dataset} \\ \hline 
RANSAC & 70.26 & 46.45 \\
Label Transformation & 67.17 & 43.44 \\ \hline
...
+ RANSAC & 72.02 & 48.53 \\ 
+ Label transformation & 69.90 & 49.37 \\
+ Tuned & 70.59 & 48.96 \\ 
\end{tabular}
